...
const routes = {
  ...
  "/handoffs/restore": (b, x) => ({ ok: b.restoreHandoff(x.id) }),
  "/handoffs/trash": (b, x) => ({ handoffs: b.listTrash(x) }),
  "/handoffs/purge": (b, x) => ({ ok: b.purgeHandoff(x.id) }),
  "/handoffs/empty-trash": (b) => ({ deleted: b.emptyTrash() }),
